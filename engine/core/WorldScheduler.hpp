...
public:
    ...
    void destroy(const World::Entity entity) { commands_->destroy(entity); }

    template <typename T> void remove(const World::Entity entity) {
  ...
  private:
    ...
    explicit CommandWriter(WorldCommandBuffer &commands) noexcept
        : commands_(&commands) {}
    WorldCommandBuffer *commands_ = nullptr;
  };

  using Callback = void (*)(void *context, World &world,
                            CommandWriter &commands, const InputState &input,
                            double elapsed, double delta);
  using ParallelCallback = void (*)(void *context, const World &world,
                                    const InputState &input, double elapsed,
                                    double delta);
  ...
  WorldSystemScheduler &operator=(const WorldSystemScheduler &) = delete;
    ...
    executionPhases_.reserve(systemCount);
    parallelHandles_.reserve(systemCount);
    parallelInvocations_.resize(systemCount);
  }
  ...
  void addSystem(const std::string_view name, const Callback callback,
  ...
  void addParallelSystem(const ParallelSystemDesc &descriptor) {
    ensureNotExecuting();
    validateDescriptor(descriptor);

    System system;
    system.name.assign(descriptor.name.data(), descriptor.name.size());
    system.parallelCallback = descriptor.callback;
    system.context = descriptor.context;
    system.dependencies.reserve(descriptor.dependencies.size());
    for (const std::string_view dependency : descriptor.dependencies) {
      system.dependencies.emplace_back(dependency.data(), dependency.size());
  ...
  void
  addParallelSystem(const std::string_view name,
                    const ParallelCallback callback,
                    void *const context = nullptr,
                    const std::span<const std::string_view> dependencies = {}) {
    addParallelSystem(
        ParallelSystemDesc{name, callback, context, dependencies});
  }
  ...
  void clear() {
    ...
    void compile() {
        ...
        for (const std::string &dependency : system.dependencies) {
          if (findSystem(dependency) == nullptr) {
            throw std::invalid_argument(
                "World system dependency is not registered: " + dependency);
          }
          ++indegrees_[systemIndex];
        }
      }

      for (std::size_t emittedCount = 0U; emittedCount < count;
           ++emittedCount) {
        ...
        for (std::size_t candidate = 0U; candidate < count; ++candidate) {
          ...
          for (const std::string &dependency :
               systems_[dependent].dependencies) {
      ...
      executionPhases_.clear();
      executionPhases_.reserve(count);
      parallelHandles_.reserve(count);
      parallelInvocations_.resize(count);
      for (const std::size_t systemIndex : executionOrder_) {
        const System &system = systems_[systemIndex];
        const bool parallel = system.parallelCallback != nullptr;
        bool beginPhase = executionPhases_.empty() ||
                          executionPhases_.back().parallel != parallel ||
                          !parallel;
        if (!beginPhase) {
          for (const std::size_t phaseSystem :
               executionPhases_.back().systems) {
            for (const std::string &dependency : system.dependencies) {
              if (dependency == systems_[phaseSystem].name) {
                beginPhase = true;
              }
            }
          }
        ...
        if (beginPhase) {
          executionPhases_.push_back(ExecutionPhase{parallel, {}});
        }
        executionPhases_.back().systems.push_back(systemIndex);
      }
      compiled_ = true;
    } catch (...) {
  ...
  [[nodiscard]] ExecutionResult execute(World &world, const InputState &input,
                                        const double elapsed,
                                        const double delta) {
    return executeInternal(nullptr, world, input, elapsed, delta);
  }

  [[nodiscard]] ExecutionResult execute(JobSystem &jobs, World &world,
                                        const InputState &input,
                                        const double elapsed,
                                        const double delta) {
    return executeInternal(&jobs, world, input, elapsed, delta);
  }
...
private:
  struct System {
    std::string name;
    Callback callback = nullptr;
    ParallelCallback parallelCallback = nullptr;
    void *context = nullptr;
  ...
  void validateDescriptor(const SystemDesc &descriptor) const {
    ...
    for (std::size_t dependencyIndex = 0U;
         dependencyIndex < descriptor.dependencies.size(); ++dependencyIndex) {
      const std::string_view dependency =
          descriptor.dependencies[dependencyIndex];
      if (dependency.empty()) {
        throw std::invalid_argument(
            "World system dependency name must not be empty");
      }
      for (std::size_t previous = 0U; previous < dependencyIndex; ++previous) {
        if (dependency == descriptor.dependencies[previous]) {
          throw std::invalid_argument(
              "World system dependencies must not contain duplicates");
        }
      }
    }
  }

  void validateDescriptor(const ParallelSystemDesc &descriptor) const {
    if (descriptor.name.empty()) {
      throw std::invalid_argument("World system name must not be empty");
    }
    if (descriptor.name.size() > JobTimelineEvent::kMaximumNameBytes) {
      throw std::invalid_argument(
          "Parallel World system name exceeds job timeline capacity");
    }
    if (descriptor.callback == nullptr) {
      throw std::invalid_argument(
          "Parallel World system callback must not be null");
    }
    if (findSystem(descriptor.name) != nullptr) {
      throw std::invalid_argument("World system name is already registered");
    }
  ...
  [[nodiscard]] const System *
  findSystem(const std::string_view name) const noexcept {
  ...
  static void runParallelJob(void *context, JobContext &) {
    const ParallelInvocation &invocation =
        *static_cast<const ParallelInvocation *>(context);
    invocation.system->parallelCallback(invocation.system->context,
                                        *invocation.world, *invocation.input,
                                        invocation.elapsed, invocation.delta);
  }

  void executeParallelPhase(JobSystem &jobs, const ExecutionPhase &phase,
                            const World &world, const InputState &input,
                            const double elapsed, const double delta) {
    parallelHandles_.clear();
    try {
      for (const std::size_t systemIndex : phase.systems) {
        ParallelInvocation &invocation = parallelInvocations_[systemIndex];
        invocation = {&systems_[systemIndex], &world, &input, elapsed, delta};
        parallelHandles_.push_back(
            jobs.submit({.name = systems_[systemIndex].name,
                         .callback = runParallelJob,
                         .context = &invocation,
                         .category = JobCategory::Simulation}));
      }
    } catch (...) {
      for (const JobHandle handle : parallelHandles_) {
        try {
          static_cast<void>(jobs.wait(handle));
        } catch (...) {
        }
      }
      jobs.releaseAll(parallelHandles_);
      throw;
    }

    std::exception_ptr failure;
    try {
      jobs.waitAll(parallelHandles_);
    } catch (...) {
  ...
  [[nodiscard]] ExecutionResult executeInternal(JobSystem *jobs, World &world,
                                                const InputState &input,
                                                const double elapsed,
                                                const double delta) {
    if (!compiled_) {
      throw std::logic_error(
          "World system scheduler must be compiled before execution");
    }
    if (executing_) {
      throw std::logic_error(
          "Recursive World system scheduler execution is forbidden");
    }
    if (!std::isfinite(elapsed) || elapsed < 0.0 || !std::isfinite(delta) ||
        delta <= 0.0) {
      throw std::invalid_argument(
          "World system timing must be finite, with non-negative elapsed time "
          "and positive delta time");
    }

    executing_ = true;
    for (const std::unique_ptr<detail::SimulationStepResource> &resource :
    ...
    try {
      ...
      for (const ExecutionPhase &phase : executionPhases_) {
        if (phase.parallel && jobs != nullptr) {
          executeParallelPhase(*jobs, phase, world, input, elapsed, delta);
          continue;
        }
        for (const std::size_t systemIndex : phase.systems) {
          const System &system = systems_[systemIndex];
          if (system.parallelCallback != nullptr) {
            system.parallelCallback(system.context, world, input, elapsed,
                                    delta);
          } else {
            system.callback(system.context, world, commandWriter, input,
                            elapsed, delta);
          }
        }
      }
  ...
  void ensureNotExecuting() const {
    if (executing_) {
      throw std::logic_error(
          "World system scheduler mutation during execution is forbidden");
    }
  }
  ...
  std::vector<ExecutionPhase> executionPhases_;
  std::vector<JobHandle> parallelHandles_;
  std::vector<ParallelInvocation> parallelInvocations_;
  std::vector<std::unique_ptr<detail::SimulationStepResource>>
      simulationResources_;
