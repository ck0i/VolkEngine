...
public:
  ...
  void require(const std::size_t count) const {
    if (count > input_.size() - offset_)
      throw std::runtime_error("Texture artifact is truncated");
  }
...
T readLittle(const std::span<const std::byte> bytes, const std::size_t offset) {
  ...
  for (std::size_t index = 0; index < sizeof(T); ++index) {
    value |=
        static_cast<T>(std::to_integer<std::uint8_t>(bytes[offset + index]))
        << (index * 8U);
  }
  return value;
}
...
std::uint64_t expectedMipBytes(const TextureStorage storage,
                               const std::uint32_t width,
                               const std::uint32_t height) {
  const std::uint64_t pixels = static_cast<std::uint64_t>(width) * height;
  switch (storage) {
  case TextureStorage::Rgba8:
    return pixels * 4U;
  case TextureStorage::Rgba32Float:
    return pixels * 4U * sizeof(float);
  case TextureStorage::Bc1Rgba:
    return static_cast<std::uint64_t>((width + 3U) / 4U) *
           ((height + 3U) / 4U) * 8U;
...
void validateTexture(const TextureArtifact &texture) {
  if (!texture.id.valid() || texture.width == 0U || texture.height == 0U ||
      ...
      texture.role > TextureRole::Emissive ||
  ...
  for (const TextureMip &mip : texture.mips) {
    const std::uint64_t expected =
        expectedMipBytes(texture.storage, width, height);
    if (mip.width != width || mip.height != height || mip.offset != offset ||
        mip.size != expected ||
        expected > std::numeric_limits<std::uint64_t>::max() - offset) {
  ...
  if (texture.storage == TextureStorage::Rgba32Float) {
    for (std::size_t offsetBytes = 0; offsetBytes < texture.data.size();
         offsetBytes += sizeof(float)) {
      float value = 0.0f;
      std::memcpy(&value, texture.data.data() + offsetBytes, sizeof(value));
      if (!std::isfinite(value)) {
        throw std::runtime_error(
            "Texture artifact HDR payload contains NaN or infinity");
      }
    }
  }
}
...
BlockFormat blockFormat(const std::uint32_t vkFormat) {
  ...
  case 131U:
  case 133U:
    return {TextureStorage::Bc1Rgba, TextureColorSpace::Linear};
  case 132U:
  case 134U:
    return {TextureStorage::Bc1Rgba, TextureColorSpace::Srgb};
  case 137U:
    return {TextureStorage::Bc3Rgba, TextureColorSpace::Linear};
  case 138U:
    return {TextureStorage::Bc3Rgba, TextureColorSpace::Srgb};
  case 145U:
    return {TextureStorage::Bc7Rgba, TextureColorSpace::Linear};
  case 146U:
    return {TextureStorage::Bc7Rgba, TextureColorSpace::Srgb};
  default:
    throw std::runtime_error(
        "KTX2 vkFormat is not a supported BC1/BC3/BC7 GPU format: " +
...
void appendDisjointRegion(const std::span<const std::byte> source,
  ...
  if (offset > source.size() || length > source.size() - offset) {
    throw std::runtime_error(std::string("KTX2 ") + name +
...
TextureArtifact importKtx2(const std::span<const std::byte> source,
                           const AssetId id, const TextureRole role,
                           const TextureColorSpace requestedColorSpace,
                           const TextureImportOptions &options) {
  if (source.size() < kKtx2HeaderBytes ||
      !std::ranges::equal(kKtx2Identifier,
                          source.first(kKtx2Identifier.size()))) {
  ...
  if (width == 0U || height == 0U || depth != 0U || layers > 1U ||
      faces != 1U || levelCount == 0U || levelCount > 32U || typeSize != 1U) {
    throw std::runtime_error(
        "KTX2 must be a bounded two-dimensional single-layer texture");
  }
  ...
  if (supercompression != 0U) {
    throw std::runtime_error("KTX2 supercompression requires a BasisLZ/Zstd "
                             "transcoder and is unsupported");
  }
  const BlockFormat format = blockFormat(vkFormat);
  if (format.colorSpace != requestedColorSpace) {
    throw std::runtime_error(
        "KTX2 vkFormat color space does not match the material texture role");
  }
  const std::uint64_t levelIndexEnd =
      kKtx2HeaderBytes +
      static_cast<std::uint64_t>(levelCount) * kKtx2LevelIndexBytes;
  if (levelIndexEnd > source.size())
  ...
  appendDisjointRegion(source, occupied, kvdOffset, kvdLength,
                       "key/value data");
  ...
  for (std::uint32_t level = 0; level < levelCount; ++level) {
    ...
    appendDisjointRegion(source, occupied, sourceOffset, sourceLength,
                         "mip level");
    sourceMips.push_back({{mipWidth, mipHeight, payloadOffset, sourceLength},
  ...
  for (const SourceMip &mip : sourceMips) {
    ...
    const auto levelBytes =
        source.subspan(static_cast<std::size_t>(mip.source.offset),
                       static_cast<std::size_t>(mip.source.length));
    texture.data.insert(texture.data.end(), levelBytes.begin(),
                        levelBytes.end());
  }
...
TextureArtifact importTextureArtifact(const std::span<const std::byte> source,
                                      const std::filesystem::path &sourceName,
                                      const AssetId id, const TextureRole role,
                                      const TextureColorSpace colorSpace,
                                      const TextureImportOptions &options) {
  if (!id.valid())
    throw std::invalid_argument("Texture AssetId is invalid");
  if (source.size() > options.maximumSourceBytes) {
    throw std::runtime_error("Texture source exceeds the configured limit");
  }
  const std::string extension = lowercaseExtension(sourceName);
  if (extension == ".ktx2") {
    return importKtx2(source, id, role, colorSpace, options);
  }
  ...
  if (extension == ".hdr") {
    if (colorSpace != TextureColorSpace::Linear) {
      throw std::runtime_error(
          "HDR texture artifacts must use linear color space");
    }
    LoadedImageRgba32F image =
        loadImageRgba32F(source, sourceName.generic_string());
    ...
    LoadedImageRgba8 image =
        loadImageRgba8(source, sourceName.generic_string());
  ...
  texture.mips.push_back(
      {texture.width, texture.height, 0U, texture.data.size()});
  validateTexture(texture);
  return texture;
}

TextureArtifact importTextureArtifact(const std::filesystem::path &path,
                                      const AssetId id, const TextureRole role,
                                      const TextureColorSpace colorSpace,
                                      const TextureImportOptions &options) {
  const std::vector<std::byte> source =
      readBinaryFile(path, options.maximumSourceBytes);
  return importTextureArtifact(source, path, id, role, colorSpace, options);
}
...
serializeTextureArtifact(const TextureArtifact &texture) {
  ...
  writer.output.insert(writer.output.end(), texture.data.begin(),
                       texture.data.end());
  return std::move(writer.output);
}

TextureArtifact
deserializeTextureArtifact(const std::span<const std::byte> bytes) {
  Reader reader{bytes};
  ...
  texture.colorSpace =
      static_cast<TextureColorSpace>(reader.pod<std::uint8_t>());
  ...
  if (!reader.done())
    throw std::runtime_error("Texture artifact has trailing data");
  validateTexture(texture);
  return texture;
}
