vec3 applyFoliageWind(vec3 localPosition, mat4 model, uint materialBits) {
    uint materialClass = (materialBits >> 3U) & 15U;
    if (materialClass != 3U) return localPosition;

    float weight = smoothstep(0.15, 4.5, localPosition.y);
    vec2 anchor = model[3].xz;
...
vec3 applyFoliageWind(vec3 localPosition, SceneInstance instance) {
    return applyFoliageWind(
        localPosition, instance.model, instance.textureIndices.w);
}
