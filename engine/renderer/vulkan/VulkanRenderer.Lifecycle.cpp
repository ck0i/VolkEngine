...
VulkanRenderer::Impl::Impl(Window &window, EngineConfig config,
  ...
  if (!isValidExposure(config_.exposure)) {
    ...
    logger()->info(
        "Renderer enabled: dynamicRendering {} sync2 {} timestamps {} "
        "validation {} debugMarkers {} memoryBudget {} indirectSceneDraws {} "
        "imguiOverlay {} transferUploadSync {} depthMinReduction {}; "
        "supported: descriptorIndexing {} bindlessSampledImages {} "
        "multiDrawIndirect {} drawIndirectFirstInstance {} samplerAnisotropy "
        "{} samplerFilterMinmax {} computeSubgroupBallot {} maxAnisotropy "
        "{:.1f} maxDrawIndirectCount {}",
        capabilityName(deviceOwner_.info.dynamicRendering),
        capabilityName(deviceOwner_.info.synchronization2),
        capabilityName(deviceOwner_.info.timestampQueries),
        capabilityName(deviceOwner_.info.validationEnabled),
        capabilityName(deviceOwner_.info.debugMarkers),
        capabilityName(deviceOwner_.info.memoryBudget),
        capabilityName(deviceOwner_.info.indirectSceneDraws),
        capabilityName(imguiOwner_.initialized),
        ...
        capabilityName(deviceOwner_.info.bindlessSampledImagesSupported),
        capabilityName(deviceOwner_.info.multiDrawIndirect),
        capabilityName(deviceOwner_.info.drawIndirectFirstInstance),
        capabilityName(deviceOwner_.info.samplerAnisotropy),
    ...
    const GpuResourceRegistry::Stats resourceStats =
        resourceOwner_.registry.stats();
    logger()->info("Tracked GPU resources: {} live ({} buffers, {} images, {} "
                   "imported), {:.2f} MiB estimated (buffers {:.2f}, owned "
                   "images {:.2f}, imported images {:.2f})",
                   resourceStats.liveResources, resourceStats.buffers,
                   resourceStats.images, resourceStats.importedImages,
                   bytesToMiB(resourceStats.bytes),
                   bytesToMiB(resourceStats.bufferBytes),
                   bytesToMiB(resourceStats.ownedImageBytes),
...
VulkanRenderer::Impl::~Impl() { cleanupResources(true); }

void VulkanRenderer::Impl::cleanupResources(
    const bool persistPipelineCache) noexcept {
  ...
  if (frameOwner_.timestampQueryPool != VK_NULL_HANDLE) {
    vkDestroyQueryPool(deviceOwner_.device, frameOwner_.timestampQueryPool,
                       nullptr);
    ...
    if (pipelineOwner_.cache != VK_NULL_HANDLE) {
        ...
        try {
          logger()->warn(
              "Failed to save Vulkan pipeline cache during shutdown: {}",
              exception.what());
        } catch (...) {
        }
      } catch (...) {
  ...
  if (resourceOwner_.descriptorPool != VK_NULL_HANDLE) {
    vkDestroyDescriptorPool(deviceOwner_.device, resourceOwner_.descriptorPool,
                            nullptr);
  ...
  if (resourceOwner_.lightingSetLayout != VK_NULL_HANDLE) {
    vkDestroyDescriptorSetLayout(deviceOwner_.device,
                                 resourceOwner_.lightingSetLayout, nullptr);
  ...
  if (resourceOwner_.depthReductionSampler != VK_NULL_HANDLE) {
    vkDestroySampler(deviceOwner_.device, resourceOwner_.depthReductionSampler,
                     nullptr);
  ...
  if (swapchainOwner_.surface != VK_NULL_HANDLE) {
    vkDestroySurfaceKHR(deviceOwner_.instance, swapchainOwner_.surface,
                        nullptr);
    swapchainOwner_.surface = VK_NULL_HANDLE;
  }
  if (deviceOwner_.debugMessenger != VK_NULL_HANDLE) {
    const auto destroy = reinterpret_cast<PFN_vkDestroyDebugUtilsMessengerEXT>(
        vkGetInstanceProcAddr(deviceOwner_.instance,
...
void VulkanRenderer::Impl::waitIdle() {
    ...
    if (indirectSceneDrawsEnabled_ && lastFrame.completedGpuCullCountersValid) {
      stats_.sceneItemCount = lastFrame.submittedSceneItemCount;
      stats_.visibleItemCount = lastFrame.completedVisibleItemCount;
      stats_.culledItemCount = lastFrame.submittedSceneItemCount -
      ...
      stats_.testedCullingUnitCount = lastFrame.completedTestedCullingUnitCount;
      stats_.occludedCullingUnitCount =
          lastFrame.completedOccludedCullingUnitCount;
      stats_.sphereLodHighCount = lastFrame.completedSphereLodCounts[0];
    ...
    const std::uint64_t validationErrorCount =
        deviceOwner_.validationMessages.errorCount.load(
            std::memory_order_relaxed);
    if (config_.requireValidation && validationErrorCount > 0U) {
      throw std::runtime_error(
          "Strict Vulkan validation observed " +
          std::to_string(validationErrorCount) +
