...
- `engine/renderer/vulkan/VulkanRenderer.Resources.cpp` — long-lived GPU buffers/images, authored texture loading and valid per-role fallbacks for untextured generated materials, generated HDR environment and shadow-atlas images/samplers, descriptor layouts/pools/sets, tonemap descriptor setup, and resource-registry metadata.
...
- `engine/renderer/vulkan/VulkanRenderer.Visibility.cpp` — frustum extraction/culling, grid visibility acceleration, LOD bucketing, post-cull material-class telemetry, and draw-work planning.
...
5. Create authored/fallback textures, the mipmapped linear HDR environment, samplers/descriptors, pipeline cache plus scene/atmosphere pipelines, per-frame light/tile/shadow buffers, generated/imported meshes, tonemap descriptors, and timestamp queries.
...
flowchart LR
    ...
    Shadows --> ScenePre
    Atmosphere[Analytic atmosphere] --> Scene
    Atmosphere --> ScenePre
...
- Direct light uses GGX/Smith/Schlick. Specialized class branches add clear-coat, wrapped foliage/skin response, anisotropic hair, cloth sheen, emissive output, altitude/slope/moisture landscape blending, and bounded water reflection/Fresnel response without new descriptor sets or per-material pipeline permutations.
- When enabled, the HDR pass first draws a fullscreen analytic sky from camera basis, directional-light direction, and fixed atmosphere coefficients. Scene geometry then depth-tests over it. Foliage vertices apply deterministic camera-relative sinusoidal displacement from material wind amplitude and elapsed time in camera, depth, and shadow variants, so silhouettes remain consistent.
...
- Generated M2 terrain, water, grass, shrub, and tree meshes enter the same authored scene arrays before renderer construction. Stable `AssetId` ordinals resolve to normal runtime mesh handles; no landscape-only draw path exists.
...
- Accepted GPU candidates atomically increment fixed per-material-class counters next to visible/tested/occluded totals. CPU fallback counting increments only accepted items or cached tile histograms. Completed-frame counters drive `RenderStats`, idle refresh, overlays, and schema-v7 visibility evidence.
...
- ImGui and run-summary schema v6 expose bounded lighting/material state; the
  summary additionally records bounded job/IO counters, worker timing, and the
  optional residency/partition traversal block.
