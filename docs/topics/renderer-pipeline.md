...
The HDR path selects filtered B10G11R11 when supported and falls back to RGBA16F; composition does not use alpha. It uses GGX/Smith/Schlick lighting, shares each fragment's view cosine across directional, local, and ambient terms, and samples a complete octahedral environment mip chain with up to four reflection probes. Packed material classes select foliage, landscape, or water branches. Atmosphere runs after opaque shading with a reverse-Z clear-depth equality test, so covered pixels do not invoke it. The terminal 1×1 environment radiance is uploaded with lighting uniforms for diffuse reuse; roughness selects directional specular mips. Tonemapping applies exposure, ACES, and exactly one sRGB encoding step.
