...
| Visibility | Capability-gated compute performs culling, LOD, Hi-Z rejection, compaction, counters, and command generation; exact 2x pyramid levels bypass general ratio divisions, Hi-Z culling derives mip extents from uploaded base dimensions, subgroup paths aggregate visible and per-material counters before global atomics, and fixed frustum plane sets are unrolled. |
