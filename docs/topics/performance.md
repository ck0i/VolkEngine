...
| Uploads | Position, UV, and packed-surface streams share one allocation; forward vertices reuse model transforms for direction-preserving normals and transform tangents only for normal-mapped and hair materials, mesh cooks favor cache hits then low aggregate valence, 16-entry procedural cooks break ties with their two least-connected live vertices, and scene indices narrow to 16 bits when possible; meshes and textures share staging submissions. |
