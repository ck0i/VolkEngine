...
| Uploads | Position, UV, and packed-surface streams share one allocation; mesh cooks favor cache hits then retiring low-valence vertices, procedural meshes target a 16-entry window, and scene indices narrow to 16 bits when possible; meshes and textures share staging submissions. |
