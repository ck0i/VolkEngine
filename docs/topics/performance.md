...
- `engine/renderer/vulkan/VulkanRenderer.Lighting.cpp`: bounded Forward+ tile storage, per-frame light/probe upload, shadow-view/cascade preparation, and atlas pressure.
...
- Use dynamic rendering (`vkCmdBeginRendering`) for shadow-atlas, depth, HDR, and tonemap graphics work; Forward+ assignment and visibility remain compute dispatches in the same primary command buffer.
...
| Runtime residency | One bounded scheduler covers texture, geometry, world-cell, animation, and audio artifacts. Dependency expansion and stable priority dispatch share the existing `JobSystem`; frame-scoped desired/pinned sets cancel obsolete work, generational completions cannot publish stale bytes, and LRU eviction enforces one explicit resident-byte budget. | `ResidencyManager.cpp` |
| World partition | Canonical hierarchical selection uses bounds distance, prefetch margin, and coarse-frontier retention. Complete `VECW` frontiers combine transactionally, rebase around quantized origins, and publish only at a frame callback boundary; rejected candidates retain active cells. | `WorldPartition.cpp`, `Application.cpp` |
| Landscape cooking | Height/biome queries and brush edits are deterministic and bounded. Terrain vertices/indices, water planes, and foliage instances are produced once during cook; hierarchy-selected local-space patches and reusable vegetation meshes then use ordinary partition residency and GPU submission. | `Landscape.cpp`, `samples/partition/main.cpp` |
...
| Scene instances | Per-frame mapped storage starts at 2048 records and grows transactionally after its fence. Each frame slot retains canonical source items and updates candidate/instance records only where bounds, mesh, transform, material, or texture handles changed; unchanged records avoid data writes and normal-matrix recomputation, while fully unchanged frames also avoid candidate/instance host-write barriers. The direct fallback retains CPU instance materialization. | `VulkanRenderer.FrameResources.cpp`, `VulkanRenderer.Frame.cpp`, `VulkanRenderer.Visibility.cpp` |
| Visibility | Mesh upload cooks bounded clusters and ranges. Compute performs instance and optional cluster frustum tests, sphere LOD selection, conservative temporal Hi-Z rejection, visible-instance compaction, post-frustum material-class counters, and command generation. Capability-gated subgroup ballot reservation coalesces contended instance allocations; the ordinary atomic fallback retains identical bounded output partitions. CPU grid acceptance adds precomputed tile class histograms instead of revisiting items. A half-resolution reverse-Z min pyramid retains odd source edges with proportional footprints and reduced owned-image memory by about 4 MiB at 1280×720 versus a full-resolution base. | `VulkanRenderer.Visibility.cpp`, `VulkanRenderer.Frame.cpp`, `depth_pyramid.comp`, `scene_cull.comp` |
| Submission | The measured default emits one indexed indirect command per mesh and compacted instance indices. `--cluster-indirect-commands` selects finer cluster commands for diagnostic or geometry-heavy workloads; either mode submits one `vkCmdDrawIndexedIndirect` per scene pass. The direct indexed fallback remains capability/config gated and does not allocate cull buffers. | `VulkanRenderer.Frame.cpp`, `VulkanRenderer.FrameResources.cpp` |
...
| Frame graph | Cached depth/no-depth and screenshot/no-screenshot variants execute Forward+ assignment, visibility cull/command generation, shadow atlas, depth, HDR, temporal depth-pyramid build, tonemap/ImGui, and readback work directly. Imported temporal resources preserve read-before-write order; compilation derives deterministic barriers, lifetimes, activation/retirement points, and alias-compatible transient slots. | `FrameGraph.hpp`, `VulkanRenderer.Frame.cpp`, `VulkanRenderer.FrameResources.cpp` |
| Forward+ lights | One 16×16-tile compute dispatch writes a fixed partition of 64 indices per tile for up to 256 local lights. Capacity grows only after the owning frame fence; deterministic overflow is counted instead of reallocating or using a contended global append allocator. | `Lighting.cpp`, `VulkanRenderer.Lighting.cpp`, `light_assign.comp` |
| Shadows | A fixed 2048² atlas and sixteen 512² slots bound memory and rendering. Three directional cascades plus scene-order local spot assignments expose view/capacity/overflow pressure; `--no-shadows` records no shadow views. | `Lighting.cpp`, `VulkanRenderer.Lighting.cpp`, `VulkanRenderer.Frame.cpp` |
| Environment/PBR | One 256×128 linear half-float HDR image and nine CPU-built mips provide roughness LOD without runtime prefilter dispatches. Four fixed-size probe records blend the same environment resource. Ten material classes share one packed instance ABI and descriptor layout; landscape/water response and foliage wind add no per-material descriptors. Analytic atmosphere is one fullscreen HDR draw before geometry. | `VulkanRenderer.Resources.cpp`, `VulkanRenderer.Lighting.cpp`, `scene.frag`, `atmosphere.frag` |
...
| Diagnostics | Optional built-in/editor/streaming overlays and schema-v7 run summaries expose graph structure/allocation, recompiles, lighting/shadow/post-cull material visibility, bounded job scheduling, residency bytes/pressure/failures, partition coverage/publication/origin state, stable manifest/landscape content hashes, procedural geometry/biome/species counts, budgets, and a capped per-frame traversal trace. | `VulkanRenderer.ImGui.cpp`, `RunSummary.cpp`, `samples/partition/main.cpp` |
| Dynamic rendering | Graph callbacks record Forward+ assignment, visibility cull/command generation, shadow atlas, optional depth, analytic atmosphere plus HDR PBR/environment lighting, temporal Hi-Z build, tonemap/ImGui, and screenshot copy. Tracked image/buffer state skips exact no-op transitions; graph final transitions cover presentation and host readback. | `VulkanRenderer.Frame.cpp`, `VulkanRenderer.Sync.cpp` |
...
GPU timing fields are valid only when `gpuTimestampsValid` is true. `gpuLightAssignmentMs` measures the Forward+ dispatch, `gpuCullMs` measures generated visibility work, `gpuShadowMs` measures atlas work when shadows are enabled, and `gpuDepthPyramidMs` measures the post-HDR pyramid build. When the prepass is disabled by `Auto` or `--no-depth-prepass`, `gpuDepthPrepassMs` is zero and HDR timing includes the depth-writing scene pass.
...
Run-summary schema v7 retains schema-v6 renderer, jobs, and `streaming`
fields, then adds a `landscape` object. Streaming records manifest/content
SHA-256 identities, residency budget and pressure/failures, live cell state,
traversal/publication/origin/coverage totals, maximum visible
instances/triangles, and at most 4,096 observer samples. Landscape records its
seed/content identity, terrain LOD/vertex/triangle counts, biome samples,
foliage species, water/edit counts, traversal distance, atmosphere/wind state,
post-cull visible class maxima, and CPU/GPU budgets. The serializer rejects
oversized externally constructed traces rather than truncating them.
...
- `--hiz-occlusion` / `--no-hiz-occlusion` — isolate temporal occlusion build/rejection cost from GPU frustum/LOD/generated submission.
- `--auto-depth-prepass` / `--depth-prepass` / `--no-depth-prepass` — compare adaptive, forced prepass, and depth-writing HDR paths.
- `--shadows` / `--no-shadows` — isolate bounded directional/local atlas rendering from Forward+ and environment lighting.
...
- `VolkEnginePartitionBenchmark --benchmark-gate --no-vsync` — M2's 1,320-frame natural-landscape traversal. The gate requires 1,200 valid measured CPU/GPU samples, p95 within declared budgets, visible terrain/foliage/water, residency within budget, and zero coverage, partial-load, main-thread-IO, IO, OOM, or missing-dependency failure.

## Known next work

- Publish M3's controlled 4K fidelity/performance matrix on the named target hardware, including cross-driver visual policy and scene revision.
