...
| Environment/PBR | One 256×128 linear half-float HDR image and nine CPU-built mips provide roughness LOD without runtime prefilter dispatches. Four fixed-size probe records blend the same environment resource. Eight material classes share one packed instance ABI and descriptor layout. | `VulkanRenderer.Resources.cpp`, `VulkanRenderer.Lighting.cpp`, `scene.frag` |
...
| Diagnostics | Optional built-in/editor/streaming overlays and schema-v6 run summaries expose graph structure/allocation, recompiles, lighting/shadow/material/visibility state, bounded job scheduling, residency bytes/pressure/failures, partition coverage/publication/origin state, stable manifest/content hashes, and a capped per-frame traversal trace. | `VulkanRenderer.ImGui.cpp`, `RunSummary.cpp`, `samples/partition/main.cpp` |
...
Run-summary schema v6 retains the schema-v5 renderer and `job_system` fields,
then adds a `streaming` object. It records manifest/content SHA-256 identities,
the published residency budget, resident/peak/IO bytes, load/eviction/
cancellation/backpressure/failure totals, live queue/load/resident and
active/desired/pending counts, traversal/publication/origin/coverage totals,
maximum visible instances/triangles, and at most 4,096 frame samples. Each
sample includes global observer, local origin, residency pressure, cell state,
and the instantaneous coverage-gap flag. The serializer rejects oversized
traces instead of silently truncating externally constructed summaries.
...
- Establish a representative crossover for GPU-generated submission. On the current Intel RPL-S baseline, 16K and 65K grid cases remain faster through the specialized CPU grid/direct path; the GPU path reduces submitted geometry with Hi-Z but does not yet recover cull, command-granularity, and pyramid costs.
