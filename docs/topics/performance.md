...
| Uploads | Position, UV, and packed-surface streams share one geometry allocation, while scene indices narrow to 16 bits only when every referenced value fits; meshes and textures use shared staging submissions, and same-queue barriers avoid unnecessary semaphore chains. |
