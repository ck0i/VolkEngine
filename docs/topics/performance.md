...
| Scene instances | Per-frame mapped storage buffer starts at 2048 visible instances, packs CPU-precomputed normal-matrix columns for non-uniform-scale TBN correctness, and grows transactionally after the frame fence so the old mapped descriptor remains valid until the replacement buffer is mapped and descriptors are updated. Depth-prepass frames materialize directly into known per-mesh mapped ranges without allocating/populating depth sort scratch; single-pass opaque frames retain front-to-back sort keys for early-Z locality. | `VulkanRenderer.FrameResources.cpp`, `VulkanRenderer.Frame.cpp` |
...
| Submission | No-prepass frames materialize per-mesh instances in CPU scratch with one compact `{depth,index}` key per instance, sort keys front-to-back, and copy the ordered data sequentially into mapped storage. Depth-prepass frames skip both scratch arrays, per-instance depth calculation, sorting, and the extra copy by writing directly into precomputed per-mesh mapped ranges; both depth and HDR passes consume that stable order. Multi-draw indirect is used when required Vulkan feature bits are enabled; direct indexed-instanced fallback avoids allocating per-frame indirect command buffers. | `VulkanRenderer.Frame.cpp`, `VulkanRenderer.FrameResources.cpp` |
...
| Frame graph | Cached depth/no-depth and screenshot/no-screenshot variants execute depth, HDR, tonemap/ImGui, and readback work directly. Compilation derives deterministic barriers, logical lifetimes, activation/retirement points, and alias-compatible transient slots; the current depth/HDR classes intentionally occupy separate slots. | `FrameGraph.hpp`, `VulkanRenderer.Frame.cpp`, `VulkanRenderer.FrameResources.cpp` |
| Timestamps | Fixed timestamp range per frame slot reports frame, depth, HDR, and final-pass GPU durations; `gpuTimestampsValid` distinguishes real timings from pending/unavailable data. | `VulkanRenderer.FrameResources.cpp` |
...
| Diagnostics | Optional overlay is controlled by `--no-imgui` and runs only when `debugOverlay` is enabled; it refreshes stats snapshots at a fixed interval. Overlay and JSON run summaries expose graph pass/resource/barrier counts, requested/allocated transient bytes, physical slot count, recompiles, last recompile reason, and per-pass timings. | `VulkanRenderer.ImGui.cpp`, `RunSummary.cpp` |
| Dynamic rendering | The graph execution callback records the optional depth pass, HDR scene pass, tonemap/ImGui pass, and requested screenshot copy. Tracked image sync skips exact no-op transitions; graph final transitions cover presentation and host readback. | `VulkanRenderer.Frame.cpp`, `VulkanRenderer.Sync.cpp` |
