...
| Visibility | Capability-gated compute performs culling, LOD, Hi-Z rejection, compaction, counters, and command generation; exact 2x pyramid levels bypass general ratio divisions, pyramid dispatches push exact extents while Hi-Z culling derives mip extents from uploaded base dimensions, subgroup paths aggregate visible and per-material counters before global atomics, and fixed frustum plane sets are unrolled. |
| Submission | The default GPU path emits one indirect command per mesh; direct submission remains the fallback. |
| Lighting | Fixed tile and shadow-atlas partitions bound pressure; workgroups share projected light tile ranges, tile depth bounds reuse host-selected pyramid mip extents, caster culling and range checks avoid unnecessary work and reduce attenuation bounds to one-sided clamps, environment lobes share probe weights and defer common tint normalization, uploads precompute inverse-squared light ranges and probe radii, cone-width, integer probe counts, local/directional/environment radiance, and environment rotation, and each fragment precomputes material dielectric, diffuse, and wrap weights, reuses light angles for shadow bias and hair tangent normalization across lights, combines bounded Smith visibility divisions, skips back-facing half-vector work, and gates optional direct and wrapped lobes with material masks. |
| Uploads | Position, UV, and packed-surface streams share one allocation; forward vertices fetch only observed instance fields, reuse model transforms for direction-preserving normals, and transform tangents only for normal-mapped and hair materials, mesh cooks favor cache hits then low aggregate valence, 16-entry procedural cooks break ties with their two least-connected live vertices, and scene indices narrow to 16 bits when possible; meshes and textures share staging submissions. |
| Frame graph | Cached variants derive pass order, hazards, lifetimes, and compatible transient slots; overlay-free sRGB swapchains use fixed-function output encoding while overlay paths retain shader encoding. |
