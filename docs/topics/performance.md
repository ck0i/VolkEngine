...
| Uploads | Position, UV, and packed-surface streams share one geometry allocation; procedural triangle order targets a 16-entry reuse window, and scene indices narrow to 16 bits only when every referenced value fits; meshes and textures share staging submissions, and same-queue barriers avoid semaphore chains. |
