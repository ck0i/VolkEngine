...
- The sandbox resolves the cooked reference scene and RGBA8 texture artifacts through this path; startup and reload no longer decode source image bytes on the render thread. Cache/rebuild counts and cook latency remain renderer metrics, while run-summary schema v5 adds job counts, active/running work, queue high-water mark, steals, worker time, and General/Simulation/IO/Asset category totals. HDR and BC artifacts remain available for a future capability-aware Vulkan upload path.
