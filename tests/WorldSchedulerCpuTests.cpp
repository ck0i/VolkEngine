...
static_assert(
    !std::is_copy_constructible_v<ve::WorldSystemScheduler::CommandWriter>);
static_assert(
    !std::is_copy_assignable_v<ve::WorldSystemScheduler::CommandWriter>);
static_assert(
    !std::is_move_constructible_v<ve::WorldSystemScheduler::CommandWriter>);
static_assert(
    !std::is_move_assignable_v<ve::WorldSystemScheduler::CommandWriter>);

template <typename T>
concept HasPlayback =
    requires(T &commands, ve::World &world) { commands.playback(world); };

template <typename T>
concept HasDiscard = requires(T &commands) { commands.discard(); };
...
void recordSystem(void *context, ve::World &,
                  ve::WorldSystemScheduler::CommandWriter &,
                  const ve::InputState &input, const double elapsed,
                  const double delta) {
...
void queueRemoval(void *context, ve::World &,
...
void observeBeforePlayback(void *context, ve::World &world,
                           ve::WorldSystemScheduler::CommandWriter &,
                           const ve::InputState &, double, double) {
...
void recordOnce(void *context, ve::World &,
                ve::WorldSystemScheduler::CommandWriter &commands,
                const ve::InputState &, double, double) {
...
void throwOnce(void *context, ve::World &,
               ve::WorldSystemScheduler::CommandWriter &,
               const ve::InputState &, double, double) {
  auto &state = *static_cast<FailureContext *>(context);
  if (state.throwNow) {
...
void noOpSystem(void *, ve::World &, ve::WorldSystemScheduler::CommandWriter &,
                const ve::InputState &, double, double) {}

void mutateScheduler(void *context, ve::World &,
                     ve::WorldSystemScheduler::CommandWriter &,
                     const ve::InputState &, double, double) {
...
void readAndMaybePublish(void *context, ve::World &,
                         ve::WorldSystemScheduler::CommandWriter &,
                         const ve::InputState &, double, double) {
...
void publishEvent(void *context, ve::World &,
                  ve::WorldSystemScheduler::CommandWriter &,
                  const ve::InputState &, double, double) {
...
void queueCommandAndOverflow(void *context, ve::World &,
                             ve::WorldSystemScheduler::CommandWriter &commands,
                             const ve::InputState &, double, double) {
...
void queueCommandAndPublishEvent(
    void *context, ve::World &,
    ve::WorldSystemScheduler::CommandWriter &commands, const ve::InputState &,
    double, double) {
...
void rejectEventSchedulerMutation(void *context, ve::World &,
                                  ve::WorldSystemScheduler::CommandWriter &,
                                  const ve::InputState &, double, double) {
...
void observeTimers(void *context, ve::World &,
                   ve::WorldSystemScheduler::CommandWriter &,
                   const ve::InputState &, double, double) {
  auto &state = *static_cast<TimerObserveContext *>(context);
  for (const ve::SimulationTimerEvent<TimerPayload> &event :
       state.queue->events()) {
...
void scheduleTimer(void *context, ve::World &,
                   ve::WorldSystemScheduler::CommandWriter &,
                   const ve::InputState &, double, double) {
...
void rejectTimerReset(void *context, ve::World &,
                      ve::WorldSystemScheduler::CommandWriter &,
                      const ve::InputState &, double, double) {
...
void queueCommandAndObserveTimer(
    void *context, ve::World &,
    ve::WorldSystemScheduler::CommandWriter &commands, const ve::InputState &,
    double, double) {
...
struct ParallelPhaseState {
  std::atomic_uint32_t bits{0};
  std::atomic_uint32_t active{0};
  std::atomic_uint32_t maximumActive{0};
};

struct ParallelPhaseContext {
  ParallelPhaseState *state = nullptr;
  std::uint32_t requiredBits = 0;
  std::uint32_t outputBit = 0;
  bool throwNow = false;
};

void parallelPhase(void *context, const ve::World &world,
                   const ve::InputState &, double, double) {
  const auto &task = *static_cast<ParallelPhaseContext *>(context);
  if ((task.state->bits.load(std::memory_order_acquire) & task.requiredBits) !=
      task.requiredBits) {
    throw std::runtime_error("parallel phase crossed a dependency barrier");
  }
  static_cast<void>(world.componentCount<Position>());
  if (task.throwNow) {
    throw std::runtime_error("parallel system failure");
  }
  const std::uint32_t active =
      task.state->active.fetch_add(1U, std::memory_order_acq_rel) + 1U;
  std::uint32_t maximum =
      task.state->maximumActive.load(std::memory_order_relaxed);
  while (maximum < active && !task.state->maximumActive.compare_exchange_weak(
                                 maximum, active, std::memory_order_relaxed)) {
  }
  std::this_thread::sleep_for(std::chrono::milliseconds(15));
  task.state->bits.fetch_or(task.outputBit, std::memory_order_release);
  task.state->active.fetch_sub(1U, std::memory_order_release);
}

void mutablePhase(void *context, ve::World &,
                  ve::WorldSystemScheduler::CommandWriter &,
                  const ve::InputState &, double, double) {
  const auto &task = *static_cast<ParallelPhaseContext *>(context);
  if ((task.state->bits.load(std::memory_order_acquire) & task.requiredBits) !=
      task.requiredBits) {
    throw std::runtime_error("mutable phase crossed a dependency barrier");
  }
  task.state->bits.fetch_or(task.outputBit, std::memory_order_release);
}
} // namespace
...
int main() {
    ...
    expectTrue("registration invalidates scheduler plan",
               !scheduler.compiled());
    scheduler.compile();
    expectTrue("scheduler compiles registered systems", scheduler.compiled());
    expectTrue("execution order exposes every compiled system",
               scheduler.executionOrder().size() == 3U);

    const ve::WorldSystemScheduler::ExecutionResult result =
        scheduler.execute(world, input, 0.02, 0.01);
    expectTrue("stable dependency order uses registration index tie break",
               trace.count == 3U && trace.order[0] == 1 &&
                   trace.order[1] == 3 && trace.order[2] == 2);
    expectTrue("scheduler forwards fixed-step timing and input",
               std::fabs(trace.elapsed - 0.02) < 1.0e-12 &&
                   std::fabs(trace.delta - 0.01) < 1.0e-12 && trace.pressed &&
                   trace.held);
    expectTrue("empty deferred playback reports no mutations",
               result.applied == 0U && result.rejected == 0U);

    const std::size_t systemCount = scheduler.systemCount();
    expectThrows<std::invalid_argument>(
        "duplicate system names are rejected atomically",
    ...
    expectThrows<std::invalid_argument>(
        "null system callback is rejected",
        [&] { invalid.addSystem("null", nullptr); });
    const std::array<std::string_view, 2> duplicateDependencies{"missing",
                                                                "missing"};
    expectThrows<std::invalid_argument>(
        "duplicate dependency names are rejected", [&] {
          invalid.addSystem({"duplicate-dependency", &noOpSystem, nullptr,
                             duplicateDependencies});
        });
    const std::array<std::string_view, 1> emptyDependency{""};
    expectThrows<std::invalid_argument>(
        "empty dependency name is rejected", [&] {
          invalid.addSystem(
              {"empty-dependency", &noOpSystem, nullptr, emptyDependency});
        });
    ...
    expectThrows<std::invalid_argument>("compile rejects missing dependencies",
                                        [&] { missing.compile(); });
    ...
    expectThrows<std::runtime_error>("compile rejects dependency cycle",
                                     [&] { cycle.compile(); });
    expectTrue("cyclic compile publishes no plan",
               !cycle.compiled() && cycle.executionOrder().empty());
  }

  {
    ...
    scheduler.compile();
    expectThrows<std::invalid_argument>(
        "execute rejects negative elapsed time",
        [&] { (void)scheduler.execute(world, {}, -0.01, 0.01); });
    expectThrows<std::invalid_argument>(
        "execute rejects non-positive delta",
        [&] { (void)scheduler.execute(world, {}, 0.01, 0.0); });
    expectThrows<std::invalid_argument>(
        "execute rejects non-finite timing", [&] {
          (void)scheduler.execute(world, {}, 0.01,
    ...
    scheduler.addSystem(
        {"observe", &observeBeforePlayback, &context, dependencies});
    scheduler.compile();
    const auto result = scheduler.execute(commandWorld, {}, 0.01, 0.01);
    expectTrue("deferred structure remains visible to all systems in the step",
               context.laterSystemSawPosition);
    expectTrue("scheduler plays deferred commands after all systems",
               result.applied == 1U && result.rejected == 0U &&
                   !commandWorld.contains<Position>(entity));
  }

  {
    ...
    expectTrue("failed step discards deferred mutations",
               !failureWorld.contains<Health>(entity));
    const auto resumed = scheduler.execute(failureWorld, {}, 0.02, 0.01);
    expectTrue("discarded command cannot leak into a later step",
               resumed.applied == 0U && resumed.rejected == 0U &&
                   !failureWorld.contains<Health>(entity));
  }

  {
    ...
    expectThrows<std::logic_error>(
        "scheduler mutation during execution is rejected",
        [&] { (void)scheduler.execute(world, {}, 0.01, 0.01); });
    expectTrue("execution failure preserves compiled system plan",
               scheduler.compiled() && scheduler.systemCount() == 1U);
  }

  {
    ...
    expectTrue("event channel accepts FIFO payloads",
               events.publish({1, 10}) && events.publish({2, 20}) &&
                   otherEvents.publish({30}));
    expectTrue("pending event payloads are not directly visible",
               events.events().empty() && otherEvents.events().empty());
    scheduler.compile();
    (void)scheduler.execute(world, {}, 0.01, 0.01);
    const std::span<const EventPayload> promoted = events.events();
    const std::span<const OtherEventPayload> promotedOther =
        otherEvents.events();
    expectTrue("successful promotion preserves typed FIFO payloads",
               promoted.size() == 2U && promoted[0].producer == 1 &&
                   promoted[0].value == 10 && promoted[1].producer == 2 &&
                   promoted[1].value == 20 && promotedOther.size() == 1U &&
                   promotedOther[0].code == 30);
    ...
    scheduler.compile();
    (void)scheduler.execute(world, {}, 0.01, 0.01);
    expectTrue("systems cannot observe events published in the same step",
               consumer.observedCount == 0U);
    const auto firstBatch = events.events();
    expectTrue(
        "multiple producers preserve deterministic registration FIFO order",
        first.publishSucceeded && second.publishSucceeded &&
            firstBatch.size() == 2U && firstBatch[0].producer == 1 &&
            firstBatch[0].value == 11 && firstBatch[1].producer == 2 &&
            firstBatch[1].value == 22);
    first.publishSucceeded = false;
    second.publishSucceeded = false;
    (void)scheduler.execute(world, {}, 0.02, 0.01);
    expectTrue("events are consumed on the next successful step only",
               consumer.observedCount == 2U &&
                   consumer.observed[0].producer == 1 &&
    ...
    (void)scheduler.execute(world, {}, 0.01, 0.01);
    expectTrue(
        "external pre-step pending events remain invisible during that step",
        consumer.observedCount == 0U && events.events().size() == 1U &&
            events.events()[0].producer == 7);
    (void)scheduler.execute(world, {}, 0.02, 0.01);
    expectTrue("external pre-step events participate in the next successful "
               "step exactly once",
               consumer.observedCount == 1U &&
                   consumer.observed[0].value == 70 && events.events().empty());
  }

  {
    ...
    expectTrue("event channel capacity reports overflow through publish",
               events.publish({1, 1}) && events.publish({1, 2}) &&
                   !events.publish({1, 3}));
    expectThrows<std::overflow_error>(
        "event overflow latch aborts the scheduler step",
        [&] { (void)scheduler.execute(world, {}, 0.01, 0.01); });
    expectTrue("overflow failure does not promote pending events",
               events.events().empty());
    expectThrows<std::overflow_error>(
        "overflow latch remains observable until reset",
        [&] { (void)scheduler.execute(world, {}, 0.015, 0.01); });
    events.reset();
    expectTrue("event channel reset clears pending data and overflow latch",
               events.publish({3, 3}));
    (void)scheduler.execute(world, {}, 0.02, 0.01);
    expectTrue("reset channel promotes subsequent events",
               events.events().size() == 1U &&
                   events.events()[0].producer == 3);
  }

  {
    ...
    scheduler.addSystem("overflow-before-playback", &queueCommandAndOverflow,
                        &context);
    scheduler.compile();
    expectThrows<std::overflow_error>(
        "channel overflow is detected before command playback",
        [&] { (void)scheduler.execute(overflowWorld, {}, 0.01, 0.01); });
    expectTrue("overflow discards deferred commands before world mutation",
               !overflowWorld.contains<Health>(entity) &&
                   events.events().empty());
  }

  {
    ...
    scheduler.compile();
    expectTrue("callback rollback seed event publishes",
               events.publish({4, 1}));
    (void)scheduler.execute(world, {}, 0.01, 0.01);
    ...
    expectThrows<std::runtime_error>(
        "callback failure rolls event channels back",
        [&] { (void)scheduler.execute(world, {}, 0.02, 0.01); });
    ...
    expectTrue(
        "retry consumes retained current event and promotes its replacement",
        context.observedCount == 2U && context.observed[0].value == 1 &&
            context.observed[1].value == 1 && events.events().size() == 1U &&
            events.events()[0].value == 2);
    (void)scheduler.execute(world, {}, 0.04, 0.01);
    expectTrue("rolled-back pending events are neither lost nor duplicated",
               context.observedCount == 3U && context.observed[2].value == 2 &&
                   events.events().empty());
  }

  {
    ...
    scheduler.addSystem("playback-rollback", &queueCommandAndPublishEvent,
                        &context);
    scheduler.compile();
    expectTrue("playback rollback seed event publishes",
               events.publish({9, 1}));
    (void)scheduler.execute(playbackWorld, {}, 0.01, 0.01);
    context.active = true;
    playbackWorld.each<Position>([&](auto &&...) {
      expectThrows<std::logic_error>(
          "playback failure promotes event transaction before rethrow",
          [&] { (void)scheduler.execute(playbackWorld, {}, 0.02, 0.01); });
    });
    expectTrue(
        "playback failure consumes current events and promotes emitted events",
        events.events().size() == 1U && events.events()[0].value == 2);
    context.active = false;
    (void)scheduler.execute(playbackWorld, {}, 0.03, 0.01);
    expectTrue("playback failure prevents retrying events against a partially "
               "committed world",
               context.observedCount == 2U && context.observed[0].value == 1 &&
                   context.observed[1].value == 2 && events.events().empty());
  }

  {
    ...
    scheduler.reserveSimulationResources(2U);
    auto &events = scheduler.createEventChannel<EventPayload, 2U>();
    expectTrue("scheduler-owned event channel reference remains usable while "
               "scheduler lives",
               events.events().empty());
    scheduler.compile();
    auto &otherEvents = scheduler.createEventChannel<OtherEventPayload, 2U>();
    expectTrue("event channel creation invalidates compilation and retains "
               "factory ownership",
               !scheduler.compiled() && otherEvents.events().empty());

    EventSchedulerGuardContext context{&scheduler, &events, &world};
    scheduler.addSystem("event-lifecycle-guard", &rejectEventSchedulerMutation,
                        &context);
    scheduler.compile();
    ...
    expectTrue("reset guard pending batch and overflow latch are seeded",
               events.publish({5, 2}) && events.publish({5, 3}) &&
                   !events.publish({5, 4}));
    expectThrows<std::overflow_error>(
        "reset rejection preserves pending overflow state during callback",
        [&] { (void)scheduler.execute(world, {}, 0.02, 0.01); });
    expectTrue("recursive execution creation reservation and reset are "
               "rejected during execution",
               context.recursiveRejected && context.creationRejected &&
                   context.reservationRejected && context.resetRejected);
    expectTrue("rejected reset preserves the pre-existing current batch",
               events.events().size() == 1U && events.events()[0].value == 1);
    expectThrows<std::overflow_error>(
        "rejected reset leaves the overflow latch intact",
        [&] { (void)scheduler.execute(world, {}, 0.03, 0.01); });
    events.reset();
    expectTrue("reset guard replacement pending event publishes",
               events.publish({5, 4}));
    (void)scheduler.execute(world, {}, 0.04, 0.01);
    expectTrue(
        "rejected reset preserves pending events for successful promotion",
    ...
    expectTrue("scheduler owns non-default-constructible timer queues",
               timers.events().empty() && timers.currentTick() == 0U);
    scheduler.compile();
    auto &otherTimers = scheduler.createTimerQueue<TimerPayload, 2U>();
    expectTrue(
        "timer queue factory retains ownership and invalidates compilation",
    ...
    scheduler.addSystem("timer-callback-schedule", &scheduleTimer,
                        &callbackSchedule);
    scheduler.compile();
    const ve::TimerHandle delayed = timers.schedule(2U, {1});
    (void)scheduler.execute(world, {}, 1000.0, 0.125);
    expectTrue("timer scheduling uses integer steps and never fires in its "
               "scheduling step",
               delayed.valid() && callbackSchedule.scheduled &&
                   observed.observedCount == 0U && timers.currentTick() == 1U);
    const ve::TimerHandle normalizedZero = timers.schedule(0U, {2});
    (void)scheduler.execute(world, {}, 1000.125, 0.125);
    expectTrue("zero-delay timer cannot fire in the step in which it is "
               "externally scheduled",
               normalizedZero.valid() && observed.observedCount == 1U &&
                   observed.payloads[0].value == 22);
    (void)scheduler.execute(world, {}, 1000.25, 0.125);
    expectTrue(
        "zero-delay timer fires on the next integer step and one-shots honor "
        "their due boundary",
    ...
    expectTrue("pending timers count as active before their first promotion",
               timers.active(first) && timers.active(second) &&
                   timers.active(third) && timers.activeCount() == 3U);
    (void)scheduler.execute(world, {}, 0.02, 0.01);
    expectTrue(
        "equal due timers are delivered FIFO by monotonic handle",
        observed.observedCount == 3U && observed.payloads[0].value == 1 &&
            observed.payloads[1].value == 2 &&
            observed.payloads[2].value == 3 && observed.handles[0] == first &&
            observed.handles[1] == second && observed.handles[2] == third);
    expectTrue("expired one-shot handles become stale after promotion",
               !timers.active(first) && timers.activeCount() == 0U &&
                   !timers.cancel(first));
  }

  {
    ...
    (void)scheduler.execute(world, {}, 0.02, 0.01);
    (void)scheduler.execute(world, {}, 0.03, 0.01);
    expectTrue("repeating timer skips non-due integer ticks",
               observed.observedCount == 1U && timers.active(repeated));
    (void)scheduler.execute(world, {}, 0.04, 0.01);
    expectTrue("repeating timer resumes at its exact repeat cadence",
               observed.observedCount == 2U &&
                   observed.payloads[0].value == 7 &&
                   observed.payloads[1].value == 7);
  }

  {
    ...
    expectTrue("cancel accepts active and pending handles only once",
               timers.active(pending) && timers.activeCount() == 1U &&
                   timers.cancel(pending) && !timers.active(pending) &&
                   timers.activeCount() == 0U && !timers.cancel(pending) &&
                   !timers.cancel({}));
    (void)scheduler.execute(world, {}, 0.01, 0.01);
    expectTrue("canceled pending timer never reaches active storage",
               !timers.active(pending) && timers.activeCount() == 0U);
  }

  {
    ...
    scheduler.compile();
    (void)scheduler.execute(world, {}, 0.01, 0.01);
    expectTrue("external cancel accepts a committed active timer",
               timers.active(active) && timers.cancel(active));
    (void)scheduler.execute(world, {}, 0.02, 0.01);
    (void)scheduler.execute(world, {}, 0.03, 0.01);
    ...
    expectTrue("cancel cannot erase the immutable current due batch",
               observed.observedCount == 1U &&
                   observed.handles[0] == repeated && !timers.active(repeated));
    (void)scheduler.execute(world, {}, 0.03, 0.01);
    expectTrue("current-batch cancellation prevents only future repeats",
               observed.observedCount == 1U);
  }

  {
    ...
    expectTrue("timer rollback seed schedule succeeds",
               timers.schedule(1U, {13}).valid());
    scheduler.compile();
    (void)scheduler.execute(world, {}, 0.01, 0.01);
    ...
    expectThrows<std::runtime_error>(
        "callback failure rolls timer state back",
        [&] { (void)scheduler.execute(world, {}, 0.02, 0.01); });
    expectTrue("callback failure retains due timers and preserves pre-step "
               "pending schedules",
               preStepPending.valid() && observed.observedCount == 1U &&
                   observed.scheduleSucceeded && timers.currentTick() == 1U &&
                   timers.active(preStepPending) && timers.activeCount() == 2U);
    observed.scheduleOnObserve = true;
    observed.scheduleSucceeded = false;
    (void)scheduler.execute(world, {}, 0.03, 0.01);
    expectTrue("retry redelivers the due timer and commits its replacement",
               observed.observedCount == 2U &&
                   observed.payloads[0].value == 13 &&
                   observed.payloads[1].value == 13 &&
                   observed.scheduleSucceeded && timers.currentTick() == 2U &&
                   timers.active(preStepPending) && timers.activeCount() == 2U);
  }

  {
    ...
    scheduler.addSystem("timer-playback-commit", &queueCommandAndObserveTimer,
                        &context);
    expectTrue("timer playback failure seed schedule succeeds",
               timers.schedule(1U, {17}).valid());
    scheduler.compile();
    (void)scheduler.execute(playbackWorld, {}, 0.01, 0.01);
    playbackWorld.each<Position>([&](auto &&...) {
      expectThrows<std::logic_error>(
          "playback failure commits timer transaction before rethrow",
          [&] { (void)scheduler.execute(playbackWorld, {}, 0.02, 0.01); });
    });
    (void)scheduler.execute(playbackWorld, {}, 0.03, 0.01);
    expectTrue("playback failure consumes due timers rather than retrying "
    ...
    scheduler.compile();
    (void)scheduler.execute(world, {}, 0.01, 0.01);
    expectTrue("timer queue reset is rejected during scheduler transaction",
               context.resetRejected);
    timers.reset();
    expectTrue("timer queue reset clears state and restores tick zero",
               timers.currentTick() == 0U && timers.activeCount() == 0U &&
                   timers.events().empty());
    ...
    expectTrue("timer reset never aliases stale handles",
               beforeReset.valid() && afterReset.valid() &&
                   beforeReset != afterReset && !timers.cancel(beforeReset) &&
                   timers.active(afterReset));
  }

  {
    ...
    scheduler.addSystem(
        "timer-overflow-before-playback",
        +[](void *raw, ve::World &,
            ve::WorldSystemScheduler::CommandWriter &commands,
            const ve::InputState &, double, double) {
    ...
    expectTrue("timer capacity returns an invalid handle and latches overflow",
               first.valid() && second.valid() && !invalid.valid() &&
                   !timers.active(invalid));
    expectThrows<std::overflow_error>(
        "timer overflow aborts before command playback",
        [&] { (void)scheduler.execute(overflowWorld, {}, 0.01, 0.01); });
    expectThrows<std::overflow_error>(
        "timer overflow latch remains until reset",
        [&] { (void)scheduler.execute(overflowWorld, {}, 0.02, 0.01); });
    expectTrue("timer overflow discards commands before world mutation",
               !overflowWorld.contains<Health>(entity));
    timers.reset();
    const ve::TimerHandle recovered = timers.schedule(1U, {4});
    (void)scheduler.execute(overflowWorld, {}, 0.03, 0.01);
    expectTrue(
        "timer reset clears the capacity latch for subsequent scheduling",
        recovered.valid() && timers.active(recovered) &&
            overflowWorld.contains<Health>(entity));
  }

  {
    ...
    expectTrue(
        "timer schedule read and compiled execution perform no allocations",
    ...
    scheduler.addSystem("event-allocation-check", &readAndMaybePublish,
                        &context);
    scheduler.compile();
    gAllocationCount = 0U;
    gTrackAllocations = true;
    ...
    gTrackAllocations = false;
    expectTrue(
        "event publish read and compiled execution perform no allocations",
        publishWithoutAllocation && context.publishSucceeded &&
            gAllocationCount == 0U);
  }

  {
    ...
    (void)scheduler.execute(world, {}, 0.01, 0.01);
    gTrackAllocations = false;
    expectTrue("compiled scheduler execution performs no allocations",
               gAllocationCount == 0U);
  }

  {
    ParallelPhaseState state;
    ParallelPhaseContext start{&state, 0U, 1U, false};
    ParallelPhaseContext first{&state, 1U, 2U, false};
    ParallelPhaseContext second{&state, 1U, 4U, false};
    ParallelPhaseContext finish{&state, 7U, 8U, false};
    ve::WorldSystemScheduler scheduler;
    scheduler.reserveSystems(4U);
    scheduler.addSystem("parallel-start", &mutablePhase, &start);
    const std::array<std::string_view, 1> afterStart{"parallel-start"};
    scheduler.addParallelSystem(
        {"parallel-first", &parallelPhase, &first, afterStart});
    scheduler.addParallelSystem(
        {"parallel-second", &parallelPhase, &second, afterStart});
    const std::array<std::string_view, 2> afterReaders{"parallel-first",
                                                       "parallel-second"};
    scheduler.addSystem(
        {"parallel-finish", &mutablePhase, &finish, afterReaders});
    scheduler.compile();
    ...
    ve::JobSystem jobs({.workerCount = 4,
                        ...
                        .timelineCapacity = 16});
    (void)scheduler.execute(jobs, world, input, 0.01, 0.01);
    expectTrue("independent read-only systems overlap behind explicit mutable "
               "barriers",
               state.bits.load(std::memory_order_acquire) == 15U &&
                   state.maximumActive.load(std::memory_order_relaxed) >= 2U);
    const ve::JobSystemStats parallelStats = jobs.stats();
    expectTrue("parallel scheduler dispatch publishes bounded job statistics",
               parallelStats.submitted == 2U && parallelStats.succeeded == 2U &&
                   parallelStats.activeJobs == 0U);

    state.bits.store(0U, std::memory_order_relaxed);
    state.maximumActive.store(0U, std::memory_order_relaxed);
    (void)scheduler.execute(world, input, 0.02, 0.01);
    expectTrue("parallel systems retain deterministic serial fallback",
               state.bits.load(std::memory_order_acquire) == 15U &&
                   state.maximumActive.load(std::memory_order_relaxed) == 1U);
  }

  {
    ve::World failureWorld;
    const ve::World::Entity entity = failureWorld.createEntity();
    failureWorld.emplace<Position>(entity, 9);
    DeferredContext removal{entity};
    ParallelPhaseState state;
    ParallelPhaseContext failure{&state, 0U, 1U, true};
    ve::WorldSystemScheduler scheduler;
    scheduler.reserveSystems(2U);
    scheduler.reserveDeferredCommandSlots(1U);
    scheduler.addSystem("parallel-record", &queueRemoval, &removal);
    const std::array<std::string_view, 1> dependency{"parallel-record"};
    scheduler.addParallelSystem(
        {"parallel-failure", &parallelPhase, &failure, dependency});
    scheduler.compile();
    ve::JobSystem jobs({.workerCount = 2,
                        ...
                        .timelineCapacity = 8});
    expectThrows<std::runtime_error>(
        "parallel system failure propagates through the deterministic phase",
        [&] {
          (void)scheduler.execute(jobs, failureWorld, input, 0.01, 0.01);
        });
    expectTrue("parallel failure rolls back deferred structural commands",
               failureWorld.contains<Position>(entity));
    failure.throwNow = false;
    (void)scheduler.execute(jobs, failureWorld, input, 0.02, 0.01);
    expectTrue("parallel scheduler remains reusable after callback failure",
               !failureWorld.contains<Position>(entity));
  }

  {
    ParallelPhaseState state;
    ParallelPhaseContext first{&state, 0U, 1U, false};
    ParallelPhaseContext second{&state, 0U, 2U, false};
    ve::WorldSystemScheduler scheduler;
    scheduler.reserveSystems(2U);
    scheduler.addParallelSystem("capacity-first", &parallelPhase, &first);
    scheduler.addParallelSystem("capacity-second", &parallelPhase, &second);
    scheduler.compile();
    ve::JobSystem jobs({.workerCount = 1,
                        ...
                        .timelineCapacity = 4});
    expectThrows<std::runtime_error>(
        "parallel phase reports bounded job-slot exhaustion",
        [&] { (void)scheduler.execute(jobs, world, input, 0.01, 0.01); });
    expectTrue("partial phase submission drains without leaked active jobs",
               jobs.stats().activeJobs == 0U);
  }

  {
    ve::WorldSystemScheduler scheduler;
    expectThrows<std::invalid_argument>(
        "parallel registration rejects null callbacks",
        [&] { scheduler.addParallelSystem("invalid-parallel", nullptr); });
    expectTrue("invalid parallel registration leaves scheduler empty",
               scheduler.empty());
  }
  ...
  std::cerr << "World scheduler CPU tests failed: " << gFailureCount
            << " failure(s)\n";
  return 1;
}
